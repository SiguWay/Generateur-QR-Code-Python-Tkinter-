...
API_KEY_IMGBB = "0db603df345099792b8091d83b6d3c16"

def uploader_image_imgbb():
    """Permet de sélectionner et uploader une image sur ImgBB"""
    global API_KEY_IMGBB
    
    # Vérifier si la clé API est configurée
    if not API_KEY_IMGBB:
        reponse = messagebox.askyesno(
            "Clé API manquante", 
            "Vous devez configurer votre clé API ImgBB.\nVoulez-vous la configurer maintenant ?"
        )
        if reponse:
            configurer_api_key()
        return
    ...
    fichier_image = filedialog.askopenfilename(
        title="Choisissez une image à uploader",
        filetypes=[
    ...
    label_upload_status.config(text="Upload en cours...", fg="orange")
    app.update()
    
    # Envoyer la requête à l'API ImgBB directement avec le fichier
    url = "https://api.imgbb.com/1/upload"
    
    with open(fichier_image, "rb") as file:
        payload = {
            "key": API_KEY_IMGBB,
        }
        files = {
    ...
    if response.status_code == 200:
        ...
        if result.get("success"):
            ...
            # Insérer le lien dans le champ de saisie
            entry_lien.delete(0, tk.END)
            entry_lien.insert(0, image_url)
            
            label_upload_status.config(text="✓ Image uploadée avec succès !", fg="green")
            
            # Afficher le lien dans une popup
            messagebox.showinfo(
                "Upload réussi", 
                f"Votre image a été uploadée !\n\nLien: {image_url}\n\nLe lien a été ajouté au champ de saisie."
            )
        else:
            label_upload_status.config(text="✗ Échec de l'upload", fg="red")
            messagebox.showerror("Erreur", "Impossible d'uploader l'image")
    else:
        label_upload_status.config(text="✗ Échec de l'upload", fg="red")
...
# Upload image
frame_upload = tk.LabelFrame(app, text=" Upload d'image sur ImgBB ", padx=10, pady=10)
frame_upload.pack(pady=10, fill="x", padx=20)

tk.Label(frame_upload, text="Upload l'image et générez son lien:", 
         font=("Arial", 9)).pack()

btn_upload = tk.Button(
    frame_upload, 
    text="📤", 
    command=uploader_image_imgbb,
    bg="#007aff",
    fg="white",
    font=("Arial", 10, "bold")
)
